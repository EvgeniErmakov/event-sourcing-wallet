...
кошелька. Polling и ручное обновление используют единые барьеры запросов: занятый ресурс получает
одну отложенную загрузку, а старый ответ статуса не может отменить результат pause/resume.
