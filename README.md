...
Известные write-side версии хранятся отдельно для каждого UUID и не переносятся при переключении
кошелька. Polling пропускает занятый ресурс, а ручное обновление и чтение после команды оставляют
максимум одну отложенную загрузку. Неизвестный результат команды блокирует новые команды до повтора
с прежним ключом; старый ответ статуса не может отменить результат pause/resume.
