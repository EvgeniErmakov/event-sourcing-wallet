> Исторический материал этапов 01–03 о собственной реализации. В этапе 04 хранение,
> конкурентный append и доставка заменены Axon; актуальная реализация описана в [../axon.md](../axon.md).

# Event Sourcing + CQRS + eventual consistency

Этот исторический документ объясняет ветку `03-cqrs-async` на примере кошелька.
