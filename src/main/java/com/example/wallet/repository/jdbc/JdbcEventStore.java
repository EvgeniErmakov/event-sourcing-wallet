...
public class JdbcEventStore implements EventStore {
        ...
        return jdbc.query("SELECT s.current_version, " + EVENT_COLUMNS + """
            ...
            while (rs.next()) {
                ...
                if (rs.getObject("event_id") == null) {
                    throw new CorruptHistoryException("Пустой сохранённый поток");
                }
                StoredEvent event = map(rs);
                if (event.streamVersion() != expected++) {
                    throw new CorruptHistoryException("Разрыв версий потока");
                }
                result.add(event);
            }
    ...
    public void append(UUID walletId, long expectedVersion, WalletEvent event, UUID eventId,
        ...
        if (expectedVersion == 0) {
            int inserted = jdbc.update("""
                    ...
                    """, params);
            if (inserted == 0) {
                throw new WalletException(WALLET_ALREADY_EXISTS, "Кошелёк уже создан конкурентом");
            }
        }
        ...
        int inserted = jdbc.update("""
                ...
                """, params);
        if (inserted == 0) {
            throw new WalletException(VERSION_CONFLICT, "Версия события уже занята");
        }
    }

    /** {@inheritDoc} Наличие проверяется по фактам, а не по сохранённому ответу команды. */
    @Override
    ...
    /**
     * {@inheritDoc} Возвращает ASC по stream_version и обнаруживает пропуски внутри страницы.
     * Этот ограниченный запрос не используется для восстановления полного состояния.
     */
    @Override
    public List<StoredEvent> readPage(UUID walletId, long afterVersion, int limit) {
        ...
        for (StoredEvent event : page) {
            if (event.streamVersion() != previous + 1) {
                throw new CorruptHistoryException("Разрыв страницы истории");
            }
