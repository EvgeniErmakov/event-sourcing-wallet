...
public class EventSerializer {
    ...
    public EventSerializer(JsonMapper mapper) { this.mapper = mapper; }

    /** Сериализованное представление для одной INSERT; версия формата не равна версии потока. */
    public record Encoded(String eventType, int schemaVersion, String payload) { }
    ...
    public WalletEvent decode(String type, int schemaVersion, String payload) {
        ...
        try {
            ...
            if (node == null || !node.isObject()) throw new IllegalArgumentException("Payload должен быть объектом");
            switch (type) {
                case "WalletCreated" -> {
                ...
                case "WalletClosed" -> {
                    if (!node.isEmpty()) throw new IllegalArgumentException("Закрытие не содержит полей");
                }
